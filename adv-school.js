// =============================================
// CLASSROOM - 7 subjects x 30 notes
// =============================================
const SUBJECTS = {
'NLP': { emoji: '\u{1F5E3}\uFE0F', notes: [
...
"Attention: model learns which tokens matter most -> Q*K^T/sqrt(d) -> softmax -> weighted V",
...
'Algorithms': { emoji: '\u{1F9EE}', notes: [
...
"Quick sort: O(n log n) avg, O(n^2) worst - in-place, cache-friendly, use random pivot",
...
"Union-Find with rank + path compression: nearly O(a(n)) amortized per operation",
"Floyd-Warshall: all-pairs shortest paths in O(n^3) - DP on intermediate vertices",
...
'Design Paradigms': { emoji: '\u{1F3D7}\uFE0F', notes: [
...
"Builder: construct complex objects step by step - fluent API (e.g., StringBuilder)",
...
'Mobile Dev': { emoji: '\u{1F4F1}', notes: [
...
"MongoDB: JSON-like BSON documents, flexible schema, embedded docs vs references",
...
function selectSubj(k) {
    curSubj = k; noteIdx = 0;
    var picker = document.getElementById('subj-picker');
    var scene = document.getElementById('classroom-scene');
    if (picker) picker.style.display = 'none';
    if (scene) scene.style.display = 'block';
    var label = document.getElementById('cl-subj-label');
    if (label) label.innerText = SUBJECTS[k].emoji + ' ' + k;
    var crowSeat = document.getElementById('crow-seat');
    var crowItems = document.getElementById('crow-items');
...
// FIX: exitClassroom - if classroom scene is showing, go to picker; if picker is showing, go to NEU
function exitClassroom() {
    var picker = document.getElementById('subj-picker');
    var scene = document.getElementById('classroom-scene');
    // If we're in the classroom scene, go back to subject picker
    if (scene && scene.style.display !== 'none') {
        scene.style.display = 'none';
        if (picker) picker.style.display = 'block';
        return;
    }
    // If we're at the subject picker (or anything else), go back to NEU
    show('screen-neu');
}

function showNote() {
    if (!curSubj) return;
    var notes = SUBJECTS[curSubj].notes;
    var board = document.getElementById('class-board');
...
function toggleStudy() {
    ...
    if (studying) {
        ...
        if (btn) btn.innerText = 'Start Studying \u{1F4D6}';
        var msg = document.getElementById('snell-msg');
        if (msg) msg.innerText = 'Break time! \u{1F389}';
    } else {
        studying = true; studySec = 1500;
        if (btn) btn.innerText = 'Take a Break \u2615';
        var msg = document.getElementById('snell-msg');
        if (msg) msg.innerText = (typeof crowFound !== 'undefined' && crowFound) ? 'Studying together! \u{1F49B}\u{1F4DA}' : 'Grinding solo... \u{1F4DA}';
        studyInt = setInterval(function() {
            studySec--;
            if (studySec <= 0) { clearInterval(studyInt); studying = false; if (btn) btn.innerText = 'Again \u{1F4D6}'; if (msg) msg.innerText = 'Session complete! \u{1F389}'; }
            var m = ~~(studySec / 60), s = studySec % 60;
            var timer = document.getElementById('study-timer');
